...
| 구분 | 내용 |
|------|------|
| 학습 데이터(목표) | 3D 렌더 합성 데이터. 렌더 시 3D 좌표 투영으로 정답 라벨 자동 생성(수작업 0) |
| 검증 데이터(현재) | Mechanical Parts Dataset 2022 (공개, 3D 렌더 데이터 확보 전 대체 검증용) |
| 규모 | 2,250장 / 4클래스(bearing·bolt·gear·nut) / 박스 10,599개 |
| 원본 포맷 | Roboflow COCO (`_annotations.coco.json` + 이미지) |

입력이 렌더든 공개 라벨이든 이후 파이프라인은 동일. 검증 대상은 "라벨셋 → 학습 → 자동라벨 확장 → 성능 향상" 루프 자체다.
...
변환 예시 (실제 데이터, 416×416 이미지):

```
# COCO (_annotations.coco.json)                # YOLO (같은 박스, .txt 한 줄)
"category_id": 3,                              2 0.374651 0.274832 0.187236 0.197115
"bbox": [116.91, 73.33, 77.89, 82.0]           (클래스 cx cy w h, 0~1 정규화)
 (x_min, y_min, w, h 픽셀)
```

```
# 변환 산출물 예: train/labels/239_jpg.rf.9urQ...txt  (bolt 3개 + nut 4개인 이미지)
1 0.426154 0.414663 0.082115 0.242788
1 0.629219 0.403846 0.083630 0.250000
3 0.743389 0.324519 0.081202 0.120192
...
```
