...
### 5.7 도메인갭 극복 · 배경합성 증강 실측 mAP (수동 GT 40장, 단일클래스 gearbox)

사람이 직접 바운딩박스를 친 GT 40장으로 잰 실측 mAP. test1·2 = 작업대(in-domain), test3·4 = 헬기 장착(out-of-domain). copy-paste 배경합성 증강 + 재탭(retap) 라벨이 미학습 환경(t3·t4)의 도메인갭을 크게 좁힘을 확인.

- mAP50 = IoU0.5 기준, mAP50:95 = IoU0.5~0.95 평균(박스 정밀도까지). P/R은 IoU0.5·conf0.25
- 핵심: 증강 없는 baseline(plain_dashboard 등)은 out-of-domain t3·t4에서 거의 0 → copy-paste+재탭(cp_mix_aug_retap*)이 t3·t4 mAP50을 1.00까지 끌어올림
...
| plain_dashboard (증강X) | 0.479 | 0.404 | 0.94 | 0.42 | 1.00 | 0.96 | 0.00 | 0.00 | 0.00 | 0.00 |
...
(원본: 옛 results/gearbox/mAP_summary.md · mAP_results.json. 요약을 이 문서로 이관하고 폴더는 정리.)
