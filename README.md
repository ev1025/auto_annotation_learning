...
│  ├─ 8_dashboard.py             # 육안 검증 대시보드 (라벨·등록·추론·이력을 웹에서 열람)
...
```bash
# 파이프라인 전 단계를 눈으로 확인: ① 데이터셋 라벨 박스 ② 등록 미리보기
# ③ 이미지/영상 업로드 추론(박스 입힌 mp4 생성) ④ 학습 이력 타임라인
python scripts/8_dashboard.py            # http://127.0.0.1:7862 (원격이면 SSH 터널)
