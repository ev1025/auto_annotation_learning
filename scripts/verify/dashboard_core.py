...
METHODS = [
    dict(
        ...
        bullets=[],
        tech=[
            ("YOLO (yolo26s)", "부품 탐지 모델. 순수(COCO 사전학습) 가중치에서 학습"),
            ("self-training", "모델이 만든 라벨을 다시 학습에 쓰는 준지도학습 루프"),
        ],
        flow=[
...
# 게이트: 새 모델 mAP50 가 기존보다 낮으면 배포하지 않고 폐기"""),],
    ...
    dict(
        ...
        bullets=[],
        tech=[
            ("Grounding DINO", "글(프롬프트)로 지정한 물체를 찾는 개방어휘 탐지기. 텍스트·이미지 매칭 (학습 안 함)"),
        ],
        flow=[
    ...
    dict(
        ...
        bullets=[],
        tech=[
            ("Grounding DINO", "방법 2와 동일하게 프롬프트로 박스 생성"),
            ("SAM", "물체의 픽셀 마스크를 구해 박스를 경계까지 좁히는 분할 모델"),
        ],
        flow=[
    ...
    dict(
        ...
        bullets=[],
        tech=[
            ("SAM", "이미지의 물체 후보를 전부 분할"),
            ("CLIP", "이미지·텍스트를 벡터로 바꾸는 임베딩 모델. 여기선 후보 크롭을 벡터화해 참조와 비교"),
        ],
        flow=[
            {"step": "입력", "text": "이미지 + 참조 갤러리 (train 정답에서 오린 크롭, 클래스당 10장. 라벨링 대상에는 정답 미사용)"},
            {"step": "1", "text": "**후보 분할** — SAM 이 이미지의 물체 후보를 전부 분할"},
            {"step": "2", "text": "**임베딩** — 각 후보 크롭을 CLIP 으로 숫자 벡터화"},
            {"step": "3", "text": "**갤러리 매칭** — 참조 갤러리와 유사도 비교, 가장 닮은 클래스 부여"},
    ...
    dict(
        ...
        bullets=[],
        tech=[
            ("SAM", "물체 후보 분할 (방법 4와 동일)"),
            ("DINOv2", "질감·형상 특징에 강한 이미지 임베딩 모델. CLIP 을 대체해 미세 부품 구분력↑"),
        ],
        flow=[
            {"step": "입력", "text": "이미지 + 참조 갤러리 (방법 4와 동일)"},
            {"step": "1", "text": "**후보 분할** — SAM 이 물체 후보를 전부 분할"},
            {"step": "2", "text": "**임베딩** — 각 후보를 **DINOv2**(질감·형상 특징)로 벡터화 (CLIP 에서 교체)"},
            {"step": "3", "text": "**갤러리 매칭** — 참조 갤러리와 유사도 비교, 임계값 0.85"},
            {"step": "결과", "text": "정밀도 0.927 = 무검수 기준(0.87) **최초 충족** → 고정밀 달성. 1탭 참조(방법 7)의 이론적 기반이 됨"},
        ],
        gallery=None, live=False,
    ...
    dict(
        ...
        tech=[
            ("SAM", "각 사진의 물체 후보 분할"),
            ("DINOv2", "후보들을 벡터화해 '사진끼리 공통으로 나오는 물체'를 유사도로 식별"),
        ],
        flow=[
...
# -> 배경 후보도 score 가 높게 나와 오채택 (한 장면 영상에서 실패한 이유)"""),],
    ...
    dict(
        ...
        subtitle="사용자가 부품을 한 번만 클릭하면, 그 '기준 사진'과 닮은 물체를 전 프레임에서 찾아 라벨링하는 방식. 배경 오채택이 원천 차단됨.",
        bullets=[],
        tech=[
            ("SAM", "클릭 지점(포인트) 분할로 참조 크롭 확보 + 전 프레임 후보 분할"),
            ("DINOv2", "참조 크롭과 각 후보의 유사도를 재 클릭한 부품만 매칭"),
        ],
        flow=[
            {"step": "입력", "text": "등록 영상/사진 + 사용자가 부품을 **한 번 클릭** (좌표를 ref.txt 에 저장)"},
            {"step": "1", "text": "**참조 확보** — SAM 포인트 분할로 클릭 지점이 속한 물체를 오려 '기준 사진' 확보"},
