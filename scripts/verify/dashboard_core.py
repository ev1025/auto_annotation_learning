...
# 게이트: 새 모델 mAP50 가 기존보다 낮으면 배포하지 않고 폐기"""),],
    ...
    dict(
        ...
        subtitle="텍스트 제로샷은 이 데이터로 학습을 전혀 하지 않고, 영어 프롬프트(설명 문구)만으로 물체를 찾는 방식입니다. "
                 "범용 개방어휘 탐지기 Grounding DINO 가 '이미지 영역이 어느 문구와 닮았나'를 유사도로 매칭해 박스에 클래스를 붙입니다 "
                 "(글을 생성하는 LLM 이 아니라 텍스트·이미지 매칭 모델).",
        ordered=True,
        bullets=[
            "**프롬프트 준비**: 부품마다 영어 설명 문구를 미리 지정한다 (metal hex bolt screw → bolt 등 4종, 매 이미지 동일).",
            "**영역·텍스트 인코딩**: 이미지에서 물체가 있을 법한 영역 후보를 뽑고, 프롬프트 4개를 각각 숫자 벡터로 변환한다.",
            "**유사도 매칭**: 각 영역 후보와 프롬프트의 유사도를 계산해 0.35 이상만 남기고, 가장 닮은 프롬프트를 그 박스의 클래스로 정한다.",
            "**후처리**: 겹친 중복 박스(NMS)와 화면을 덮는 거대 박스를 제거한다.",
            "**채점**: 정답을 숨긴 평가셋 204장에 적용하고, 숨긴 정답과 IoU 0.5 기준으로 대조한다.",
            "**탈락 사유**: 물체 위치는 곧잘 찾지만(재현율 0.42) hex bolt·hex nut 같은 유사 부품의 클래스를 자주 틀려, 오탐이 맞춘 것의 3배 → 무검수 라벨로 부적합."],
