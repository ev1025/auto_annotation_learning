...
# 게이트: 새 모델 mAP50 가 기존보다 낮으면 배포하지 않고 폐기"""),],
    ...
    dict(
        ...
        subtitle="방법 2에 SAM 을 더해, 박스를 물체의 픽셀 경계까지 좁히면 정확도가 오를지 검증한 방식.",
        bullets=[],
        flow=[
            {"step": "입력", "text": "이미지 + 영어 프롬프트 4개 (방법 2와 동일)"},
            {"step": "1", "text": "**박스 생성** — Grounding DINO 로 프롬프트에 맞는 박스 생성 (방법 2와 동일)"},
            {"step": "2", "text": "**타이트닝** — SAM 마스크로 각 박스를 물체의 픽셀 경계까지 좁힘"},
            {"step": "채점", "text": "평가셋 204장, 숨긴 정답과 IoU 0.5 대조 → 정밀도 0.239"},
            {"step": "탈락", "text": "**탈락 사유** — 방법 2와 거의 동일(0.238 → 0.239). 맞은 박스 IoU 는 DINO 원본 0.908 > SAM 0.876 로 오히려 과수축. 병목은 박스 여백이 아니라 클래스 혼동임이 실측으로 확인됨"},
        ],
