...
def build():
    ...
    m1 = section("m1", "방법 1 · self-training 오토라벨", badge("adopt"),
        sub("① 어떤 데이터를 어떻게 사용했나") + bullets([
            "<b>방식</b>: 학습된 모델이 미라벨 이미지에 예측을 만들고, 신뢰도(conf) <b>0.6 이상만</b> 라벨로 채택해 학습 데이터에 누적",
            "<b>데이터</b>: 공개 기계부품 데이터. <b>초기 라벨셋 10~15%만</b> 학습에 사용, 나머지 미라벨 풀은 라벨을 숨겨 자동 라벨 대상으로 사용",
            "<b>채점</b>: 별도 평가셋에서 라운드0(초기 라벨만) vs 라운드1(자동 라벨 추가) 성능 비교"]) +
